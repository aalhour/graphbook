...
\label{chap:graph_data_structures}
...
\noindent
What is the next task to be completed?  Our daily lives are littered
with priorities.  There are tasks that take higher priorities than
everything else and should be completed as soon as possible.  Other
tasks are of lower priorities and can be done whenever we have
time.  If we assign a nonnegative whole number to each task that needs
to be completed, the task with the highest priority has the lowest
whole number assigned to it.  As each new task comes along, it is
assigned a numerical priority.  At any point in time, we want to
choose and complete the task with the highest priority.  This chapter
will show how a structure called a priority queue can be used to
efficiently choose the next task of highest priority.  The structure
can efficiently handle from a few tasks up to millions of tasks.
...
Section~\ref{sec:graph_data_structures:priority_queues} provides some
theoretical underpinning of priority queues and considers a simple
implementation of priority queues as sorted lists.
Section~\ref{sec:graph_data_structures:binary_heaps} discusses how to
...
in section~\ref{sec:graph_data_structures:binomial_heaps} by a priority
queue implementation called binomial\index{binomial heap} heaps. As a
further application of binary\index{binary tree} trees,
section~\ref{sec:graph_data_structures:binary_search_trees} discusses
...
\label{sec:graph_data_structures:priority_queues}

A \emph{priority queue} is a queue data structure with various rules
on how to access and manage elements of the queue.  Recall from
section~\ref{subsec:graph_algorithms:breadth_first_search} that an
ordinary queue $Q$ has the following basic functions for accessing and
...
$Q$. Algorithm~\ref{alg:graph_data_structures:priority_queue_sort}
presents pseudocode of our discussion. The runtime of
Algorithm~\ref{alg:graph_data_structures:priority_queue_sort} depends
on how the priority queue $Q$ is implemented.

\begin{algorithm}[!tbp]
\input{algorithm/graph-data-structures/priority-queue-sort.tex}
\caption{Sorting a sequence via priority queue.}
\label{alg:graph_data_structures:priority_queue_sort}
\end{algorithm}
...
\label{subsec:graph_data_structures:sequence_implementation}
...
\label{sec:graph_data_structures:binary_heaps}
\index{binary heap}
...
\label{subsec:graph_data_structures:insertion_sift_up}
...
heap-order property.
Algorithm~\ref{alg:graph_data_structures:binary_heap_insert} presents
...
\input{algorithm/graph-data-structures/binary-heap-insert.tex}
\caption{Inserting a new internal vertex into a binary heap.}
\label{alg:graph_data_structures:binary_heap_insert}
\end{algorithm}
...
\label{subsec:graph_data_structures:deletion_sift_down}
...
Algorithm~\ref{alg:graph_data_structures:binary_heap_delete} summarizes
...
Algorithm~\ref{alg:graph_data_structures:binary_heap_delete} can be
...
\begin{algorithm}[!htbp]
\input{algorithm/graph-data-structures/binary-heap-delete.tex}
\caption{Extract the minimum vertex of a binary heap.}
\label{alg:graph_data_structures:binary_heap_delete}
\end{algorithm}
...
\begin{algorithm}[!htbp]
\input{algorithm/graph-data-structures/binary-heap-heapify.tex}
\caption{Heapify a binary tree.}
\label{alg:graph_data_structures:heapify_binary_tree}
\end{algorithm}
...
Algorithm~\ref{alg:graph_data_structures:heapify_binary_tree}.

Earlier in this section, we claimed that
Algorithm~\ref{alg:graph_data_structures:heapify_binary_tree} can be
...
time for Algorithm~\ref{alg:graph_data_structures:heapify_binary_tree}
is
%%
\begin{align*}
...
\label{sec:graph_data_structures:binomial_heaps}
\index{binomial heap}
...
Algorithm~\ref{alg:graph_data_structures:binomial_heap_minimum_vertex}
summarizes our discussion.

\begin{algorithm}[!htbp]
\input{algorithm/graph-data-structures/binomial-heap-minimum.tex}
\caption{Determine the minimum vertex of a binomial heap.}
\label{alg:graph_data_structures:binomial_heap_minimum_vertex}
\end{algorithm}
...
Algorithm~\ref{alg:graph_data_structures:link_roots}.

\begin{algorithm}[!htbp]
\input{algorithm/graph-data-structures/binomial-heap-link-roots.tex}
\caption{Linking the roots of binomial heaps.}
\label{alg:graph_data_structures:link_roots}
\end{algorithm}
...
Algorithm~\ref{alg:graph_data_structures:merge_root_lists} for
pseudocode of the procedure just described.

\begin{algorithm}[!htbp]
\input{algorithm/graph-data-structures/binomial-heap-merge-root-lists.tex}
\caption{Merging two root lists.}
\label{alg:graph_data_structures:merge_root_lists}
\end{algorithm}

\begin{algorithm}[!htbp]
\input{algorithm/graph-data-structures/binomial-heap-merge.tex}
\caption{Merging two binomial heaps.}
\label{alg:graph_data_structures:merge_binomial_heaps}
\end{algorithm}
...
Algorithm~\ref{alg:graph_data_structures:link_roots}, producing $B_1$
...
Algorithm~\ref{alg:graph_data_structures:link_roots} to produce
...
Algorithm~\ref{alg:graph_data_structures:merge_binomial_heaps}, which
...
Algorithm~\ref{alg:graph_data_structures:merge_binomial_heaps} is
...
Algorithm~\ref{alg:graph_data_structures:binomial_heap_insert} for
pseudocode of this straightforward procedure.

\begin{algorithm}[!htbp]
\input{algorithm/graph-data-structures/binomial-heap-insert.tex}
\caption{Insert a vertex into a binomial heap.}
\label{alg:graph_data_structures:binomial_heap_insert}
\end{algorithm}
...
\label{sec:graph_data_structures:binary_search_trees}
...
Algorithm~\ref{alg:graph_data_structures:binary_search_tree_locate}. Note
...
Algorithm~\ref{alg:graph_data_structures:binary_search_tree_locate}, if
...
\begin{algorithm}[!htbp]
\input{algorithm/graph-data-structures/binary-search-tree-locate.tex}
\caption{Locate a key in a binary search tree.}
\label{alg:graph_data_structures:binary_search_tree_locate}
\end{algorithm}
...
Algorithm~\ref{alg:graph_data_structures:binary_search_tree_minimum_key}
...
\begin{algorithm}[!htbp]
\input{algorithm/graph-data-structures/binary-search-tree-minimum.tex}
\caption{Finding a vertex with minimum key in a BST.}
\label{alg:graph_data_structures:binary_search_tree_minimum_key}
\end{algorithm}
...
Algorithm~\ref{alg:graph_data_structures:binary_search_tree_successors}.
...
\begin{algorithm}[!htbp]
\input{algorithm/graph-data-structures/binary-search-tree-successors.tex}
\caption{Finding successors in a binary search tree.}
\label{alg:graph_data_structures:binary_search_tree_successors}
\end{algorithm}
...
of $T$.
Algorithm~\ref{alg:graph_data_structures:binary_search_tree_insert}
...
\begin{algorithm}[!htbp]
\input{algorithm/graph-data-structures/binary-search-tree-insert.tex}
\caption{Inserting a vertex into a binary search tree.}
\label{alg:graph_data_structures:binary_search_tree_insert}
\end{algorithm}
...
\begin{algorithm}[!htbp]
\input{algorithm/graph-data-structures/binary-search-tree-delete.tex}
\caption{Deleting a vertex from a binary search tree.}
\label{alg:graph_data_structures:binary_search_tree_delete}
\end{algorithm}
...
Algorithm~\ref{alg:graph_data_structures:binary_search_tree_delete},
...
%% Algorithm~\ref{alg:graph_data_structures:binary_search_tree_locate} is
...
%% Algorithm~\ref{alg:graph_data_structures:binary_search_tree_locate} has
...
%% Algorithm~\ref{alg:graph_data_structures:binary_search_tree_insert} to
...
%% Algorithm~\ref{alg:graph_data_structures:single_left_rotation}. The
...
%% Algorithm~\ref{alg:graph_data_structures:single_right_rotation} for
%% pseudocode.
...
%% Algorithm~\ref{alg:graph_data_structures:right_left_rotation} for
...
%% refer to Algorithm~\ref{alg:graph_data_structures:left_right_rotation}
...
%% Algorithm~\ref{alg:graph_data_structures:AVL-insert}. If $h$ is the
...
%% \begin{algorithm}[!htbp]
%% \input{algorithm/graph-data-structures/AVL-single-left-rotation.tex}
%% \caption{Single left rotation in the trinode restructure process.}
%% \label{alg:graph_data_structures:single_left_rotation}
%% \end{algorithm}

%% \begin{algorithm}[!htbp]
%% \input{algorithm/graph-data-structures/AVL-single-right-rotation.tex}
%% \caption{Single right rotation in the trinode restructure process.}
%% \label{alg:graph_data_structures:single_right_rotation}
%% \end{algorithm}

%% \begin{algorithm}[!htbp]
%% \input{algorithm/graph-data-structures/AVL-right-left-rotation.tex}
%% \caption{Double rotation: right rotation followed by left rotation.}
%% \label{alg:graph_data_structures:right_left_rotation}
%% \end{algorithm}

%% \begin{algorithm}[!htbp]
%% \input{algorithm/graph-data-structures/AVL-left-right-rotation.tex}
%% \caption{Double rotation: left rotation followed by right rotation.}
%% \label{alg:graph_data_structures:left_right_rotation}
%% \end{algorithm}

%% \begin{algorithm}[!htbp]
%% \input{algorithm/graph-data-structures/AVL-insert.tex}
%% \caption{Insert a vertex into an AVL tree.}
%% \label{alg:graph_data_structures:AVL-insert}
%% \end{algorithm}
...
%% Algorithm~\ref{alg:graph_data_structures:binary_search_tree_delete} for
...
%% $n > 1$ vertices. Apply
%% Algorithm~\ref{alg:graph_data_structures:binary_search_tree_delete} to
...
%% Algorithm~\ref{alg:graph_data_structures:AVL-insert}; see
%% Algorithm~\ref{alg:graph_data_structures:AVL-delete} for
...
%% Algorithm~\ref{alg:graph_data_structures:AVL-delete} has worst-case
%% runtime of $O(\lg n)$.

%% \begin{algorithm}[!htbp]
%% \input{algorithm/graph-data-structures/AVL-delete.tex}
%% \caption{Delete a vertex from an AVL tree.}
%% \label{alg:graph_data_structures:AVL-delete}
%% \end{algorithm}
...
\item Let $Q$ be a priority queue of $n > 1$ elements, given in
  ...
  section~\ref{subsec:graph_data_structures:sequence_implementation},
...
\item Prove the correctness of
  Algorithms~\ref{alg:graph_data_structures:binary_heap_insert}
  and~\ref{alg:graph_data_structures:binary_heap_delete}.

\item Describe a variant of
  Algorithm~\ref{alg:graph_data_structures:binary_heap_delete} for
  modifying the key of the root of a binary heap, without extracting
  any vertex from the heap.

\item Section~\ref{subsec:graph_data_structures:insertion_sift_up}
  ...
  Algorithm~\ref{alg:graph_data_structures:binary_heap_insert}.

\item Section~\ref{subsec:graph_data_structures:deletion_sift_down}
  ...
  than Algorithm~\ref{alg:graph_data_structures:binary_heap_delete}.

\item Let $S$ be a sequence of $n > 1$ real numbers. How can we use
  algorithms described in
  section~\ref{sec:graph_data_structures:binary_heaps} to sort $S$?

\item The binary heaps discussed in
  section~\ref{sec:graph_data_structures:binary_heaps} are properly
  ...
  section~\ref{sec:graph_data_structures:binary_heaps} for managing
...
\item Numbers of the form $\binom{n}{r}$ are called
  ...
  Algorithm~\ref{alg:graph_data_structures:generate_all_r_combinations}
...
\item A problem related to the $r$-combinations of the set
  ...
  $S$. Algorithm~\ref{alg:graph_data_structures:generate_all_permutations}
...
%% \begin{algorithm}[!htbp]
%% \input{algorithm/graph-data-structures/generate-all-r-combinations.tex}
%% \caption{Generating all the $r$-combinations of $\{1, 2, \dots, n\}$.}
%% \label{alg:graph_data_structures:generate_all_r_combinations}
%% \end{algorithm}

%% \begin{algorithm}[!htbp]
%% \input{algorithm/graph-data-structures/generate-all-permutations.tex}
%% \caption{Generating all the permutations of $\{1, 2, \dots, n\}$.}
%% \label{alg:graph_data_structures:generate_all_permutations}
%% \end{algorithm}
...
\item The algorithms described in
  section~\ref{sec:graph_data_structures:binomial_heaps} are formally
...
\item Fibonacci heaps~\cite{FredmanTarjan1987} allow for amortized
...
\item Are the algorithms presented in
  section~\ref{sec:graph_data_structures:binary_search_trees} able to
...
\item The Fibonacci\index{Fibonacci!sequence} sequence $F_n$ is
  ...
  Binet\index{Binet!Jacques Philippe Marie}, even though Abraham de
